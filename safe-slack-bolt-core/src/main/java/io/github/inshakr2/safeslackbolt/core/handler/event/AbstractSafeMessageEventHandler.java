...
public abstract class AbstractSafeMessageEventHandler implements SafeBoltHandler {

    private static final Logger log = LoggerFactory.getLogger(AbstractSafeMessageEventHandler.class);
    ...
    public final Response handleSafely(EventsApiPayload<MessageEvent> payload, EventContext ctx) {
        try {
            ...
            log.error("Safe message event handler failed. pattern={}, message={}", getPattern().pattern(), e.getMessage(), e);
